/**
 * 宿題1+2: Cloud Deploy でカナリアデプロイ + 承認ステップ
 *
...
resource "google_clouddeploy_target" "prod" {
   ...
   * 宿題2: 承認ステップ
   *
   * これを true にすると、リリースを作っても
   * **人が承認するまでロールアウトが始まらない**。
   *
   *   gcloud deploy rollouts approve [ロールアウト名] ...
   *
   * ★ どこに付けるか ★
   * パイプラインではなく **ターゲット** に付ける。
   * 「本番ターゲットだけ承認が要る、ステージングは自動」
   * という分け方ができるようになっている。
   *
   * ★ AWSとの対応 ★
   * CodePipeline の手動承認アクション(Manual approval)に相当する。
   * AWSではパイプラインの中にステージとして挟んでいたが、
   * GCPはデプロイ先の属性になっている。
   */
