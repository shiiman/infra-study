...
/**
 * 宿題2-2: 大阪リージョン用の Cloud Router / Cloud NAT
 *
 * Cloud NATはリージョン単位のリソースなので、
 * リージョンを増やしたらその分だけ作る必要がある。Cloud Routerも同様。
 */
resource "google_compute_router" "router_osaka" {
  name    = "${var.user_name}-osaka-router"
  network = google_compute_network.vpc.id
  region  = "asia-northeast2"
}

resource "google_compute_router_nat" "nat_osaka" {
  name   = "${var.user_name}-osaka-nat"
  router = google_compute_router.router_osaka.name
  region = "asia-northeast2"
