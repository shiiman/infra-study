...
待っている間に S18(自動化するなら)と S19(MIG)の話をすると尺が埋まる。
時間が押しているときは、先に全員にコマンドを流してもらってから解説に入るとよい。

> **検証済み(2026-08-28)**: 実測 6分46秒。`curl localhost` で
> `Hello, Infra Study` が返ることを確認。
...
2時間のうち17分が待ちになる。解説を挟める順序に組んであるが、
