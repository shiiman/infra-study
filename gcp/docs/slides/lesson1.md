...
- **スライド**: 93枚(S01〜S93。セクション区切り8枚を含む)
...
| 今日のアジェンダ | 2分 | S12 |
| インフラ基礎の要点 | 18分 | S13〜S39 |
| GCPについて | 13分 | S40〜S48 |
| Cloud IAM | 16分 | S49〜S60 |
| Cloud Shell | 13分 | S61〜S66 |
| Terraform | 11分 | S67〜S77 |
| ハンズオン | 22分 | S78〜S86 |
| まとめ・宿題・注意事項 | 10分 | S87〜S93 |

> 押した場合の削り所: S42(リソースカテゴリ一覧)と S74(data/module ブロック)は口頭のみで飛ばせる。
> S15〜S38 のインフラ基礎は事前配布資料(GCP版スキップスライド)に逃がしてあるので、
> 遅れていれば S39(おさらい)だけで追いついてよい。
>
> **S65(Terraformのインストール)は飛ばせない。** Cloud Shell に Terraform が
...
GCP版は ACA を上級枠として3本立てにする。第9回(3/25)で試験対策をやって、その場で申込みまで済ませる。
...
### S34〜S38 | (AWS版詳細スライドの流用)

AWS版 第1回「インフラ基礎」の詳細スライドを、デッキの構成に合わせて流用する。
原稿の本文はスライドに載せるものだけを摘記する。

| スライド | 内容 | AWS版からの流用 |
| --- | --- | --- |
| S34 | (ネットワーク詳細) | AWS版 第1回「ネットワーク」の続き |
| S35 | (IPアドレス詳細) | AWS版 第1回「IPアドレス」の続き |
| S36 | (IPアドレスクラス) | `192.168.0.1/24` 分解図 |
| S37 | (ポート番号詳細) | 22/80/443/3306/6379 の表 |
| S38 | (ドメイン詳細) | AWS版 第1回「ドメイン」 |

> **制作メモ**: S34〜S38 は AWS 版の該当スライドをそのまま複製し、
> タイトルだけ GCP 向けに差し替える場合がある。本文は AWS 版を準用。
...
ネットワークには WAN と LAN がある
LAN の中での住所が IP アドレス。パブリックとプライベートがある
CIDR を意識してネットワーク設計をする(この勉強会は 172.16.0.0/16)
ポート番号が分かれば「どのプログラムに通信しようとしているか」が分かる
IP アドレスを人間に分かりやすくするのが DNS
SSL/TLS 証明書でブラウザ通信のセキュリティを担保する
```
...
**[図版]** **新規作成**。左から右へ3ステップのフロー図。
各ステップの下に作られるリソースのアイコンを配置。
**Step1(gcloud)は Terraform 管理外であることを明示する**。

```
 Step1          Step2             Step3
gcloudで     サービスアカウント →  なりすませる
GCSバケット   を作る              ようにする
(管理外)       google_service_account   …iam_member
                                なりすまし 失敗 → 成功
```
...
★ バケットは gcloud で作ったので terraform destroy の対象外
   「自分のtfstateが乗っているバケットを destroy が消しにいく」
   という事故が起きない
