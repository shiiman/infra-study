...
**[図版]** なし。**スクリーンショットは不要**(2026-09-18 判断)。
**講師が自分の画面を共有して、その場で起動を実演する。**
...
**[図版]** なし。**スクリーンショットは不要**(2026-09-18 判断)。
エラー文言は Cloud Shell 実機で確認済みで、本文にテキストで載せてある。
当日は実際に受講者の画面に出るので、それを見ながら話す。
...
## 4. 開催前に撮るスクリーンショット — **全て不要になった(2026-09-18)**

当初4枚を挙げていたが、**撮影が必要なものは無い**と判断した。

| | 当初の想定 | 判断 |
|---|---|---|
| S44 | リソース階層の画面 | **作図で対応済み。** スクショではなく描き起こす指示だった(第1回の作図3枚のうちの1枚) |
| S64 | Cloud Shell 起動アイコン / ターミナル | **不要。講師が自分の画面を共有して実演する** |
| S82 | 権限借用の失敗エラー | **不要。** 2026-09-18 に Cloud Shell 実機で文言を確認済みで、スライド本文にテキストで載っている |
| S85 | 権限借用の成功出力 | **不要。** 同上。そもそもスライド側は `[図版]` を要求していなかった |

## 5. 事前に配布するもの — **無い(2026-09-18)**

当初「インフラ基礎の自習資料」と「アンケートフォームのURL」を挙げていたが、
**どちらもデッキの中にあるので、別途用意する配布物は無い。**

- **インフラ基礎の自習資料** = **S22〜S39 の18枚**。デッキに入っている。
  当日は飛ばして各自で読んでもらう(時間配分の注記を参照)
- **アンケートフォームのURL** = **S89 に載っている**
