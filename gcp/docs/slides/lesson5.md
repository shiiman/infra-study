...
> **★ この回から、片付けが「受講者1回 + 講師1回」になる ★**
>
> この回の destroy は **Direct VPC egress の予約IPが解放されるまで
> サブネットが消せない**ため、1回では終わらない(1〜2時間かかる)。
> 以前は受講者に「2〜3時間待って打ち直す」をやらせていたが、
> **夕方〜夜に当たるのでやらない人が必ず出る。**
> 残骸があると**次回の `0. before` が同名リソースの衝突で落ちて、
> 冒頭で全員が止まる。**
>
> **受講者は講義中の1回だけ。2回目は翌朝、講師がまとめて流す。**
>
> ```
> # まず dry-run(既定)。何が消えるかを確認する
> gcp/tools/cleanup-lesson.sh 5 --names-file=<受講者名簿> \
...
> ```
>
> **受講者の tfstate を使って destroy する**ので実体とズレない。
> tfstate の置き場所は命名規則で決まっている
> (`<プロジェクトID>-tfstate-<受講者名>` / prefix `lesson<N>`)ので、
> **名簿さえあれば講師が全員分に到達できる。**
>
> `--var-file` に渡す共通値は `gcp/tools/cleanup.tfvars.example` を参照。
>
> 流したあとの確認:
>
> ```
...
**[図版]** 第1回 S06 と同じスライドを複製。今日の行(12月24日)にマーカーを移す。
...
★ 第1回 S51 で「allUsers は事故の元」と言ったやつ
...
★★ 皆さんは 1 と 2 まででOKです ★★
   ...
   3 の解放待ちがあるので、この場では終わりません。
   4 は講師がまとめてやります。翌日に打つ必要はありません。

★ 1 と 2 のあと残るのは VPC とサブネットだけで、課金はありません
   当日の夜に見張らなくて大丈夫です

★ 気になる人は自分で確認できます
     gcp/tools/check-leftover.sh [自分の名前]
