...
> **★★ 差し替える前に、そのページに装飾図形が無いか必ず確認する ★★**
>
> **2026-09-18 に第1回 p65 / p84 を壊した。**
> この2枚にはコードブロックの背景として**矩形が手で置かれていた**
> (p65に4つ、p84に2つ)。矩形は固定座標なので、
> **本文の行数が変わるとテキストと背景がずれる。**
>
> 本文だけ差し替えて済むのは、BODY プレースホルダ以外に図形が無いページだけ。
>
> ```python
> sl = pres["slides"][page-1]
> deco = [e for e in sl["pageElements"]
>         if not e.get("shape", {}).get("placeholder")]
> if deco:
