...
| 3 | 済 `docs/slides/lesson3.md` (56枚) | 済 `lesson3/` 0.before + 4ステップ | 済 syukudai1-3 | **未** |
| 4〜10 | 未 | 未 | 未 | 未 |

- Terraform コードは全 23 ディレクトリで `terraform fmt` 差分なし /
  `terraform validate` 成功(google provider 8.0.0)
- **第3回は実環境での apply が未実施**。前提として (1) DNSの委譲完了、
  (2) コードのGitHubへのpush(`0. before` をモジュール参照するため)が必要。
  確認項目は `docs/slides/lesson3.md` の付録Bに記載
