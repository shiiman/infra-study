...
variable "ecs_task_autoscale_iam_role_settings" { type = map(string) }

resource "aws_iam_role" "ecs_task_autoscale_iam_role" {
  name               = "Cloud9-${var.user_name}-ecs-task-autoscale-iam-role"
  assume_role_policy = var.ecs_task_autoscale_iam_role_settings["assume_role_policy"]
}

resource "aws_iam_role_policy_attachment" "ecs_task_autoscale_iam_role_policy_attachment" {
  role       = resource.aws_iam_role.ecs_task_autoscale_iam_role.id
  policy_arn = var.ecs_task_autoscale_iam_role_settings["policy_arn"]
}
